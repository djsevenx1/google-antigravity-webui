  ...
  try {
    ...
    if (json.access_token) {
      ...
      try {
        ...
        const idx = accounts.findIndex(a => a.tokenData?.token?.refresh_token === rf);
        if (idx >= 0) {
          accounts[idx].tokenData = tokenData;
          saveAccounts(accounts);
        }
        const active = readActiveToken();
        if (active?.token?.refresh_token === rf) {
...
export async function ensureValidToken(tokenData) {
  ...
  // token 过期 → 后台异步刷新,不阻塞当前请求(返回旧 token)
  // 同一时间只刷一次(去重)
  if (!_refreshingToken) {
    const rf = tokenData.token.refresh_token;
    if (rf) {
      _refreshingToken = refreshAccessToken(tokenData).finally(() => { _refreshingToken = null; });
    }
  }
  return tokenData; // 返回当前 token(可能过期,但 agy 内部会自己刷)
}
...
export function updateAccountQuota(email, quotaSummary, quotaBuckets) {
  ...
  if (idx >= 0) {
    if (quotaSummary) accounts[idx].quotaSummary = quotaSummary;

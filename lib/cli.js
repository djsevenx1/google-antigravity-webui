...
export async function pluginAction(action, arg) {
  ...
  if (arg && String(arg).trim()) {
    args.push(String(arg).trim());
  }
  const r = await run(args, { timeoutMs: 60000 });
  const text = (r.stdout + '\n' + r.stderr).trim();
  const bad = /error|fail|sign in|invalid|not found|unknown/i.test(text) && !/^All done|installed|enabled|disabled|uninstalled|imported/i.test(text);

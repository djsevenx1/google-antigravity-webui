...
const friendlyToolTip = (name, input) => {
  const toolMap = {
    view_file: '正在读取文件...',
  ...
  return toolMap[name] || `正在调用工具 (${name})...`;
};
  ...
  const flushLine = (line) => {
    ...
    if (stepUpdate) {
      ...
      if (stepUpdate.tool_info && stepUpdate.tool_info.name) {
        ...
        let parsedInput = stepUpdate.tool_info.input || stepUpdate.tool_info.args || stepUpdate.tool_info.parameters || {};
        ...
        const tip = friendlyToolTip(tName, lastToolInput);
        if (tName === 'ask_question' && parsedInput) {
          ...
          onProgress({
            ...
            toolInput: parsedInput,
            rawInput: lastToolInput,
            toolAction: stepUpdate.tool_info.toolAction || (parsedInput && parsedInput.toolAction) || '',
            toolSummary: stepUpdate.tool_info.toolSummary || (parsedInput && parsedInput.toolSummary) || ''
          });
        }
      } else if (onProgress) {
        let tip = '';
        const thoughtContent = stepUpdate.thought || stepUpdate.thinking || stepUpdate.content || '';
        if (stepUpdate.step_type === 'checkpoint') tip = '正在同步会话历史...';
        else if (stepUpdate.step_type === 'agent_response' || stepUpdate.step_type === 'planner_response') tip = '正在思考与组织回答...';
        if (tip) {
          onProgress({
            tip,
            stepType: stepUpdate.step_type || 'thought',
            toolName: 'thought',
            toolSummary: 'Thought for a few seconds',
      ...
      } else if (typeof stepUpdate.content === 'string' && stepUpdate.step_type === 'agent_response' && !stepUpdate.tool_info) {
        // 部分三方模型返回整段 content
        if (!prevResponse) {
    ...
    if (obj && (obj.event === 'result' || obj.event === 'done') && obj.result && (obj.result.status === 'SUCCEEDED' || obj.result.status === 'DONE' || obj.result.status === 'FAILED')) {

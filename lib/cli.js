...
export async function cliProvider({ model, messages, onDelta, onProgress, onConversationId, onAskUser, signal, effort, conversationId, permissions }) {
  ...
  let resolvePromise;
  const exitPromise = new Promise((resolve) => { resolvePromise = resolve; });
  ...
  child.onData((d) => {
    ...
    while ((nl = outBuffer.indexOf('\n')) !== -1) {
      ...
      flushLine(line);
      // 如果解析到了最终 result 事件或完成包，立即解除等待，绝不在 PTY 退出上阻塞！
      if (line.includes('"event":"result"') || line.includes('"status":"SUCCESS"') || line.includes('"state":"DONE","step_type":"agent_response"')) {
        settleTurnNow();
      }
    }
  });

  child.onExit(() => settleTurnNow());

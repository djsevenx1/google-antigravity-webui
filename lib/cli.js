...
async function runCliOnce({ model, prompt, finalConvId, onDelta, onProgress, onConversationId, onAskUser, signal, effort, permissions }) {
  ...
  try {
    ...
    if (toggle === 'yes' || toggle === 'on' || toggle === 'true') {
      proxyEnv = {
        ALL_PROXY: 'socks5://127.0.0.1:19999',
        HTTPS_PROXY: 'socks5://127.0.0.1:19999',
        HTTP_PROXY: 'socks5://127.0.0.1:19999',
  ...
  console.error('[cli] spawn env ALL_PROXY=' + (process.env.ALL_PROXY || 'NONE') + ' proxyEnv=' + (proxyEnv.ALL_PROXY || 'none'));
  const child = nodePty.spawn(bin(), args, {
    ...
    LANG: 'en_US.UTF-8', LC_ALL: 'en_US.UTF-8', LANGUAGE: 'en:en', TZ: 'America/New_York',
    ...proxyEnv,

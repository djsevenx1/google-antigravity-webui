...
export async function cliProvider({ model, messages, onDelta, onProgress, onConversationId, onAskUser, signal, effort, conversationId, permissions }) {
  ...
  // 如果包含了压缩前缀，说明需要真压缩（放弃旧 conversationId，把整个压缩块当做 prompt）
  const isCompacted = messages.some(m => m.content && m.content.includes('<CONTEXT_SUMMARY>'));
  
  let prompt = '';
  let finalConvId = String(conversationId || '').trim();

  if (isCompacted) {
    // 把有效的压缩上下文全部拼成一次新提问，同时清空会话 ID 以强制在云端开启全新上下文
  ...
  const resetChildFinishTimer = () => {
    if (finishTimer) clearTimeout(finishTimer);
    // 当已经收到实质性的助手回答文本后，若 1.2 秒内不再有任何子进程输出，主动结束并返回
    if (prevResponse && prevResponse.trim().length > 10) {

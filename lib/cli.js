...
async function runCliOnce({ model, prompt, finalConvId, onDelta, onProgress, onConversationId, onAskUser, signal, effort, permissions }) {
  ...
  const flushLine = (line) => {
    ...
    if (stepUpdate) {
      ...
      if (tName) {
        ...
        const activeCid = stepUpdate.conversation_id || activeConversationId || conversationIdSeen || finalConvId;
        if (activeCid) {
        ...
        if (tName === 'ask_question' && parsedInput) {
          ...
          onProgress({
            ...
            toolState,
            duration,
            toolAction: toolInfo.toolAction || (parsedInput && parsedInput.toolAction) || '',
            toolSummary: toolInfo.toolSummary || (parsedInput && parsedInput.toolSummary) || ''
          });
        }
      } else if (onProgress) {
        let tip = '';
  ...
  if (!prevResponse) {
    // 最终兜底：从 transcript_full.jsonl 读取模型实际已生成的完整正文回复
    const activeCid = activeConversationId || conversationIdSeen || finalConvId;
    if (activeCid) {
      const candidatePaths = [

...
export async function cliProvider({ model, messages, onDelta, onProgress, onConversationId, onAskUser, signal, effort, conversationId, permissions }) {
  ...
  const flushLine = (line) => {
    ...
    if (payload) {
      ...
      // 官方 result 事件包到达，标志着当前轮次 100% 完结，立即 0ms 结算退出并触发 done
      settleChildAndFinish();
      return;
    }
    ...
    if (stepUpdate) {
      if (stepUpdate.step_type === 'agent_response' && stepUpdate.state === 'DONE') {
        // agent_response 已经 DONE，300ms 快速安全收尾兜底
        setTimeout(() => {
          if (!hasFinished) settleChildAndFinish();
        }, 300);
      }
      ...
      if (stepUpdate.state === 'DONE' && (stepUpdate.step_type === 'agent_response' || stepUpdate.step_type === 'planner_response')) {
        settleChildAndFinish();
      }
    }
  ...
  const settleChildAndFinish = () => {
    ...
    if (resolveFinishPromise) resolveFinishPromise();
  };

  const resetChildFinishTimer = () => {
    if (finishTimer) clearTimeout(finishTimer);
    if (prevResponse && prevResponse.trim().length > 0) {
      // 已经输出了回答文本，若 800ms 内无新输出，立即收尾触发 done
      finishTimer = setTimeout(() => {
        settleChildAndFinish();
      }, 800);
    } else {
      // 纯思考/长命令阶段若 30 秒无任何输出，兜底收尾
      finishTimer = setTimeout(() => {
        settleChildAndFinish();
      }, 30000);
    }
  ...
  child.onExit(() => settleChildAndFinish());
  // 兜底保护：若子进程超过 180s 仍未结束，强制收尾
  setTimeout(() => settleChildAndFinish(), 180000);

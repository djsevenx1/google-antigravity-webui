...
function spawnCliLogin() {
  const env = { ...process.env, SSH_CONNECTION: process.env.SSH_CONNECTION || 'remote', NO_COLOR: '1' };
  if (hasScript()) {
    // 用 PTY 强制 OOB，便于抓取授权 URL 并贴回 code。
    // bin() 路径可能含空格（如 HOME 含空格），必须在传给 script 的命令串里加引号，否则 script 会把路径拆成多个 token而直接失败。
    return spawn('script', ['-qec', `'${bin()}' --print ping`, '/dev/null'], { env, stdio: ['pipe', 'pipe', 'pipe'] });
  }
  return spawn(bin(), ['--print', 'ping'], { env, stdio: ['pipe', 'pipe', 'pipe'] });
}
...
function makeLogin(id) {
  return {
    ...
    child: null,
    url: null,
    urlReady: false,     // 已捕获授权 URL，可展示给用户
    codeReady: false,    // 正在等待用户贴回 code
    output: '',
    settled: false,
    createdAt: Date.now()
  };
...
export async function cliLoginStart() {
  ...
  for (const l of logins.values()) {
    if (!l.settled && l.url) return { ok: true, id: l.id, url: l.url };
  }
...
export function cliLoginStatus(id) {
  ...
  if (login.settled) {
    ...
    if (!bad) invalidateCliAuth(); // 登录刚成功，立刻清登录态缓存
    return { status: bad ? 'error' : 'success', output: out };
  }
...
function cleanupLogin(id) {
  const login = logins.get(id);
  if (login && login.child && !login.child.killed) {
    try { login.child.kill('SIGKILL'); } catch (_) {}
  }
  logins.delete(id);
}

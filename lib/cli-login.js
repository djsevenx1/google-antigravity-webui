...
import { writeActiveToken, addAccount, setActiveAccountEmail } from './accounts.js';
...
export async function cliLoginStart() {
  ...
  child.on('close', (code) => {
    if (code !== 0 && !login.error) {
      const lines = login.output.split('\n').map(s => s.trim()).filter(Boolean);
      const errLine = lines.slice().reverse().find(l => /error|fail|invalid|timeout|denied|malformed|handshake/i.test(l));
      login.error = errLine || lines.slice(-2).join(' ') || `CLI 退出码 ${code}`;
    }
    finalize();
  });
  child.on('error', (e) => {
  ...
  if (!login.url) {
    const tail = login.output.slice(-400).trim();
    cleanupLogin(id);
    return { ok: false, error: login.error || tail || 'CLI 未输出授权链接（可能 eligibility/网络失败）' };
  }
...
export function cliLoginStatus(id) {
  ...
  if (login.settled) {
    ...
    const hasValidToken = !!(tokenData?.token?.access_token || tokenData?.token?.refresh_token);

    if (hasValidToken) {
      invalidateCliAuth();
      writeActiveToken(tokenData);
      addAccount('', tokenData).then(r => {
        if (r && r.ok && r.account?.email) {
          setActiveAccountEmail(r.account.email);
        }
      }).catch(() => {});
      if (login.fakeHome) {
        try { fs.rmSync(login.fakeHome, { recursive: true, force: true }); } catch (_) {}
      }
      return { status: 'success', output: login.output, tokenData };
    }
    ...
    if (!errorMsg) {
      ...
      if (errLine) {
        ...
        errorMsg = 'CLI 未输出授权链接（可能网络连接 Google 超时）';
      } else {
        errorMsg = '授权 Code 验证失败或已过期，请重新获取并重试';
      }
    }

    if (/invalid_grant|malformed/i.test(login.output) || /invalid_grant|malformed/i.test(errorMsg)) {
      errorMsg = '授权 Code 无效或已失效（Code 为一次性有效），请重新打开授权链接并复制最新 Code';
    } else if (/timeout|handshake/i.test(login.output) || /timeout|handshake/i.test(errorMsg)) {
      errorMsg = '连接 Google 服务器超时（TLS/网络受限），请检查旁路由或网络代理设置';
    }

    return { status: 'error', output: login.output, error: errorMsg };
  }

...
let cachedGoogleProfile = {
  ...
  name: 'Developer',
  picture: 'https://lh3.googleusercontent.com/a/generic-avatar-hash=s96-c',
      ...
      if (fs.existsSync(tp)) {
        ...
        if (token) {
          ...
          cachedGoogleProfile = {
            ...
            picture: profile.picture || 'https://lh3.googleusercontent.com/a/generic-avatar-hash=s96-c',
            tier: tierData.name,
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (rawData) => {
    ...
    let existingRun = activeRuns.get(convKey);
    if (existingRun) {
      debugLog(`[ws/chat] attach to run ${convKey} (isRunning=${existingRun.isRunning}, done=${existingRun.done}, events=${existingRun.events.length})`);
      if (existingRun.done) {
        // run 已完成：不回放事件（历史已在 localStorage/服务端持久化），直接返回 idle
        try { ws.send(JSON.stringify({ idle: true })); } catch (_) {}
        return;
      }
      if (existingRun.isRunning) {
